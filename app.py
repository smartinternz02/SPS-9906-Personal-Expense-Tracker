...
app.config['MYSQL_USER'] = 'NXyiv6co8a'
app.config['MYSQL_PASSWORD'] = '5aBufpTacc'
app.config['MYSQL_DB'] = 'NXyiv6co8a'
mysql = MySQL(app)
        ...
        if data:
            if session['s_m']:
                ...
                if (total['tsum']>bud):
                   ...
                   TEXT =' Dear'+' '+ session['username'] + ' You have exceeded your monthly budget of amount'+' '+str(session['budget'])+ ', For the month of'+' '+session['s_m']+'.You current expenses are worth:'+session['total']+'Yours Truely,Ex-pense Tracker'
                   message  = 'Subject: {}\n\n{}'.format("Ex-Pense Tracker", TEXT)
                   sendmail(TEXT,email)
                   sendgridmail(email,TEXT)
...
def email_transaction():
    ...
    with open('transaction.csv', 'rb') as f:
        ...
        f.close()
    message = Mail(
    from_email='poojapuranikmath71@gmail.com',
    to_emails=em['email'],
    ...
    sg = SendGridAPIClient('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')
